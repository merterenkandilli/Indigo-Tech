...
def extract_unseen_person(used_ids,detections_track):
    ...
    for used in used_ids:
        personid = used
        center = detections_track[personid]
        new_center_points[personid] = center
...
cap = cv2.VideoCapture('stock-footage-business-people-entering-office-trough-security-gates.webm') # used to capture video
trackerName = 'csrt'
opencv_trackers = {
...
while True:
    ...
    cv2.putText(image_np_with_detections, 'ROI in:', (int(width/2-30), 140), font, 1, (255, 0, 0), thickness, cv2.LINE_AA)
    cv2.rectangle(image_np_with_detections, (int(width/2-30), 150), (400, 200), (255, 0, 0), 1)
    cv2.putText(image_np_with_detections, 'ROI out:', (100, 140), font, 1, (0, 0, 255), thickness,cv2.LINE_AA)
    cv2.rectangle(image_np_with_detections, (100, 140), (230, 200), (0, 0, 255), 1)
