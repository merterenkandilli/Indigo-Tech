...
def extract_unseen_person(used_ids,detections_track):
    '''used_ids stores ids of detected person in ROI if person is unseen its extracted from
...
def check_new_person(counts,boxes,detections,detections_track,used_ids):
    for i in range(10):
        if detections['detection_scores'][0][i].numpy()>0.5:
     '''tensorflow returns the coordinates in normalized manner so we need to multiply with width and height.'''
            ...
            if 150<ymin<170:
                ...
                # we add the coordinate of detection to tracker to track the person
                tracker = opencv_trackers[trackerName]()
                trackers.add(tracker, ROI_frame, (x, y, w, h))
            '''opencv tracker predicts next location of object and compares stored detections it simply checks the eucledian distance between detected center point and tracked center point and decides whether its a new person'''
                for coordinates in boxes:
                    x, y, w, h = coordinates
                    cx = (x + x + w) // 2
                    cy = (y + y + h) // 2
                    person_detected_before = False
                    '''new found persons and old persons ids are stored in used_ids.'''
                    ...
                    if person_detected_before is False:
                        ...
                        counts += 1
    return counts,used_ids,detections_track
...
while True:
    ...
    (ret1, boxes) = trackers.update(ROI_frame) #.update returns the new location of tracked object
    trackers.clear()
    trackers = cv2.legacy.MultiTracker_create() 
    counts,used_ids,detections_track=check_new_person(counts,boxes,detections,detections_track,used_ids)
    ...
    cv2.putText(image_np_with_detections, 'ROI:', (20, 190), font, 1, (255, 0, 0), thickness, cv2.LINE_AA)
    cv2.rectangle(image_np_with_detections, (0, 150), (595, 200), (255, 0, 0), 1)
